<template>
    <div>
        <transition name="fade" mode="out-in">
            <!--If success isnt true, then it means the user hasnt sent the edit-->
            <div v-if="!success" key="create">
            ...
            <div class="row pt-2 d-flex justify-content-center">
                <button class="btn btn-primary" type="submit" :disabled="inProgress">Guardar Cambios
                </button>
            </div>
        </div>
    </div>
...
<div v-else key="success">
    <classified-success  :mode="'edit'" :classified="classified" :image="classified.images[0].URI"></classified-success>
</div>
</transition>
</div>
</template>
...
<script>
    export default {
        ...
        created(){
            ...
            this.classifiedCopy = JSON.parse(JSON.stringify(this.classified));
        },

        methods: {
            sendClassified() {
                if (JSON.stringify(this.classified) === JSON.stringify(this.classifiedCopy))
                {
                    alert("No se han realizado cambios en la publicación")
                } else {
                    this.classified.images = this.images;
                    if (!this.inProgress) {
                        this.inProgress = true;
                        if (this.deletedFlag){
                            axios.delete('/image', {data: {images: this.deletedImages}});
                        }
                        //Extra prevention in case the user somehow deletes the only image
                        if(this.images[0] != undefined) {
                            axios.put('/classified/' + this.classified.id, {
